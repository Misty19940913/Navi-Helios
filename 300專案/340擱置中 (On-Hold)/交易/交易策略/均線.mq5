...
//--- check signals
   ...
   // 檢查開倉信號
   if(rt[0].open>ma[0] && rt[0].close<ma[0])  // K棒向下穿越均線
   {
      // 如果有持有多頭，先平掉多頭
      if(hasLong)
      {
   ...
   else if(rt[0].open<ma[0] && rt[0].close>ma[0] && !hasLong)  // K棒向上穿越均線
   {
      signal = ORDER_TYPE_BUY;  // 開多頭倉位
   }
