...
- User wants external services (GitHub, GitLab, Stripe, CI/CD, IoT, monitoring) to trigger agent runs via webhooks
- User needs to forward webhook events to Telegram, Discord, or other chat platforms
- User wants push notifications from external APIs without polling
- User needs event-driven automation where an external service POSTs data and expects an intelligent response
- User asks to "subscribe to" an external service's events
- User mentions setting up a webhook listener or receiver
- User wants to pipe external events into Hermes for processing
