...
metadata:
  hermes:
    ...
    triggers:
      - "build a RAG system with DSPy"
      - "create an agent with tools using DSPy"
      - "optimize prompts automatically with DSPy"
      - "build a multi-stage AI pipeline"
      - "use chain of thought reasoning in DSPy"
      - "declare an LM program with signatures"
      - "compile a DSPy module with BootstrapFewShot"
      - "set up DSPy with Claude or GPT-4"
      - "build a question answering system with DSPy"
      - "use ProgramOfThought in DSPy"
      - "implement self-consistency in DSPy"
      - "add assertions to DSPy modules"
