...
trigger: "When the user wants to run local GGUF models, use llama.cpp for inference, find or select quantized model files on Hugging Face, or needs CPU/GPU inference with llama.cpp (including Apple Silicon, CUDA, ROCm, or Intel GPUs)."
metadata:
  hermes:
