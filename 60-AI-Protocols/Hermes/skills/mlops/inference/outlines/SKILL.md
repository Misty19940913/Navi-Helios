...
trigger: "When the user asks about Outlines, structured JSON/regex/Pydantic generation, grammar-based token constraints, type-safe LLM outputs, FSM-constrained generation, or using local models (Transformers/llama.cpp/vLLM) for guaranteed valid output formats."
metadata:
  hermes:
