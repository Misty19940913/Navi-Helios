...
### 陷阱 8：Vault 知識研究的 Ghost Notes 問題

**現象**：MOC 中有條目，但磁碟上沒有實際檔案。浪費時間嘗試讀取不存在的檔案。

**研究 vault 時的正確順序**：
1. 先列出 MOC 中所有 wiki link
2. 對每個 link 做 `read_file` 驗證檔案是否存在
3. 存在的才讀，不存在的跳過並記錄為「ghost note」
4. 從 ghost note 的 MOC 描述中提取知識框架（作為已知前提，不依賴實際內容）

**驗證脚本**：
```bash
# 從 MOC 取出 wiki link → 逐一 read_file 驗證
# 參考：skill-authoring/references/vault-trading-knowledge-audit.md
```

**預防**：任何 vault 研究任務，第一個動作是先執行上述驗證流程，再開始閱讀。
