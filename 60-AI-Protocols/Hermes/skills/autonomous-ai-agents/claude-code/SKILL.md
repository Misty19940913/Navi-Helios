...
13. **Comparing safety mechanisms across systems — verify the layer, not just the effect** — When asked "can we implement X from doc Y in system Z", `permissions.deny` (Claude Code, LLM-layer policy) and `pre_tool_call` hooks (Hermes, tool-execution-layer gate) look similar but work at different layers. The former prevents the LLM from generating a dangerous tool call; the latter intercepts after the LLM already decided to call it.

    **Concrete example from 2026-05-05:** The "Claude Code 安全三件套" doc proposes `rm → trash` alias + 20-item deny list + permission modes. Hermes has HARDLINE_PATTERNS (unconditional block, like `rm -rf /`) and DANGEROUS_PATTERNS (confirm prompt, like `git reset --hard`). Comparing the two:
    - The doc's "第二層" 20-item deny list maps to Hermes's built-in patterns — but Hermes's list is already present and more comprehensive (includes `sudo`, gateway self-termination protection, etc.). **Do not claim "not implemented" without checking the existing pattern library first.**
    - The doc's "第一層" (`rm` → trash) has **no** Hermes equivalent — this is genuinely missing.
    - The doc's "第三層" permission modes map partially to Hermes `approvals.mode` (`off/prompt/bypass`), but "Accept Edits" mode from the doc has no direct Hermes equivalent.

    Before recommending implementation of a safety feature from external docs, check: (1) is it already built into Hermes/HARDLINE_PATTERNS/DANGEROUS_PATTERNS? (2) does it map to an existing Hermes config option? (3) what is genuinely absent and needs adding?
