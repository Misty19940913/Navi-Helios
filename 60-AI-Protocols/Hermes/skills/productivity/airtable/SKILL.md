...
- Wants to read, create, update, delete, or search Airtable records
- Asks to sync data between Airtable and another system
- Needs to find records by formula or filter conditions
- Wants to batch-import records into Airtable
- Asks to list bases, tables, or schema information
- Mentions "Airtable", "airtable", or a base/table ID (`app...` / `tbl...`)
- Wants to upsert records by a merge field (idempotent sync)

**Trigger phrases:** `airtable`, `Airtable record`, `sync to airtable`, `upsert airtable`, `airtable base`, `airtable table`, `list airtable`, `find in airtable`, `delete airtable record`
