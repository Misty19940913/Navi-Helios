...
### WSL/Terminal Failure Workaround

**Symptom**: Every `terminal` command fails with:
```
FileNotFoundError: [Errno 2] No such file or directory: '/mnt/c/Users/.../Obsidian/Obsidian/3-Knowledge Base（知識庫）/理財'
```
The error path is clearly corrupted (`Obsidian/Obsidian` duplication, wrong working directory).

**Root Cause**: The `terminal` tool initializes with a corrupted `$PWD` or shell state — every command starts from the wrong directory.

**Workaround**: Use `execute_code` (Python `os` module) instead of `terminal` for filesystem exploration:
```python
import os
path = "/mnt/c/Users/安泰/OneDrive/Obsidian/300專案/320進行中 (Active)/AI"
os.listdir(path)          # list directory
os.path.exists(path)      # check existence
os.stat(full_path)        # get file info
open(fpath, encoding='utf-8')  # read files
```

Also try: `terminal(..., workdir='/')` or `terminal(..., workdir='/tmp')` to override the broken default.
