...
function migrateFile(filePath) {
    ...
    if (yamlMatch) {
        ...
        if (pMatch) {
            ...
            // Fix previous bug and ensure multi-value is handled or single wikilink is quoted
    ...
    if (!yamlObj.time_create) {
        try {
            yamlObj.time_create = fs.statSync(filePath).birthtime.toISOString().split('T')[0];
        } catch (e) {
            yamlObj.time_create = yamlObj.time_modifie;
        }
    }
...
files.forEach(f => {
    ...
    const newContent = migrateFile(filePath);
    if (newContent) {
